...
def parse_nor(alpha, k):
    ...
    gene = []
    assert PRIMITIVES_nor[-1] == 'none'  # assume last PRIMITIVE is 'none'
...
def parse_red(alpha, k):
    ...
    gene = []
    assert PRIMITIVES_red[-1] == 'none'  # assume last PRIMITIVE is 'none'

    # 1) Convert the mixed op to discrete edge (single op) by choosing top-1 weight edge
    # 2) Choose top-k edges per node by edge score (top-1 weight in edge)
    for edges in alpha:
        # edges: Tensor(n_edges, n_ops)
        #     edge_max, primitive_indices = torch.topk(edges[:, 1:-1], 1) # ignore 'none' and 'skip'
        # else:
        edge_max, primitive_indices = torch.topk(
            edges[:, :], 1)  # ignore 'none'
        topk_edge_values, topk_edge_indices = torch.topk(edge_max.view(-1), k)
        ...
        for edge_idx in topk_edge_indices:
            prim_idx = primitive_indices[edge_idx]
            prim = PRIMITIVES_red_0[prim_idx]
            node_gene.append((prim, edge_idx.item()))
